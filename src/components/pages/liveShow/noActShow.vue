<template>
	<div>
		...
		<loadlist @ondataupdate="getdata"  @onempty="showEmpty" :apiurl="'/YearCard/getConsumeActivityList/type/activity/pageSize/20/cardId/0/p/@p'"></loadlist>
	...
	methods: {
		getdata(datas) {
			datas.activityList.forEach(function(item,index,self) {
				item.actId = '/YearCard/publishShow/id/' +item.id+"/type/activity/source/detail";
				item.activityId = '/bj/acd' +item.id;
